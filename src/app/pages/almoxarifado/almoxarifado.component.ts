...
import { AlmoxarifadoFormComponent } from '../../components/almoxarifado-form/almoxarifado-form.component';
import { MaterialCardComponent } from '../../components/material-card/material-card.component';

@Component({
  selector: 'app-almoxarifado',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule, AlmoxarifadoFormComponent, MaterialCardComponent],
  templateUrl: './almoxarifado.component.html',
  styleUrls: ['./almoxarifado.component.css']
})
export class Almoxarifado implements OnInit {
  ...
  excluir(materialId: string): void {
    const material = this.materiais.find(m => m.id === materialId);
    const confirmar = confirm(`Excluir ${material?.nome ?? 'material'}?`);
